...
public class ResponseConfiguration : EntityConfiguration<Response>
{
    ...
    public override void Configure(EntityTypeBuilder<Response> builder)
    {
        ...
        builder.HasOne(x => x.Prompt).WithMany(x => (ICollection<Response>) x.Responses).HasForeignKey(x => x.PromptId);
        builder.HasOne(x => x.Ai).WithMany(x => (ICollection<Response>) x.Responses).HasForeignKey(x => x.AiId);
    }
}
